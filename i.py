...
csrf_token = re.search('"csrf_token":"(.*?)"', response.text)
if csrf_token:
...
while True:
    ...
    for username in usernames:
        ...
        # Save HTML content of the page to a .html file
        html_file_path = f"{username}_page.html"
        with open(html_file_path, "w", encoding="utf-8") as html_file:
            html_file.write(response.text)
        print(f"Saved HTML for {username} to {html_file_path}")

        # Attempt to locate the shared data (updated method)
        script_match = re.search(r'window\._sharedData = ({.*?});', response.text)
        
        if script_match:
            script = script_match[1]
            ...
            if not is_private:
                ...
                print("otherwise i will check the same username again and again\n")
                print("it is up to you\n")

        else:
            print(f"Could not find the shared data for {username}")
